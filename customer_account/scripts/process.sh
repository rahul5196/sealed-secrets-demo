...
SCRIPT_DIR_AWK="$(cd "$(dirname "$0")" && pwd)"
...
do
  ...
  if $(is_it_an_insert "$key"); then
    ...
    printf "%s\n" "$line" | jq -c -S . | awk --csv -f "$SCRIPT_DIR_AWK/generate_ins_or_upd.awk" -vTNAME=$TABLENAME -vtype=insert -vKEY=$KEY
  else
    ((update_count++))  # Increment update count
    r1=$(lookup_row_in_postgres "$key" | jq -S .)
    diff --unchanged-line-format= --old-line-format= --new-line-format='%L' <(echo "$r1") <(echo "$line") | paste -s -d ' ' | sed 's/,$//' | awk --csv -vid="$key" -f "$SCRIPT_DIR_AWK/generate_ins_or_upd.awk"  -vTNAME=$TABLENAME -vtype=update -vKEY=$KEY -vSCHEMA_NAME=$SCHEMA_NAME
  fi
